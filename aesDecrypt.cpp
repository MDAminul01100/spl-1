...
#define Multiply(x,y) (((y & 1) * x) ^ ((y>>2 & 1) * xtime(xtime(x))) ^ ((y>>1 & 1) * xtime(x)) ^ ((y>>3 & 1) * xtime(xtime(xtime(x)))) ^ ((y>>4 & 1) * xtime(xtime(xtime(xtime(x))))))
...
int main()
{
    ...
    while(1)
    {
        cout << "Lenth of key(128,192,256):";
        cin >> keyLenth ;
        if(keyLenth == 128 || keyLenth == 192 || keyLenth == 256)
            break;
    }
    Nk = keyLenth/32;
    Nr = Nk + 6;
